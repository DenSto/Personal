...
data_iter = 1000 # or if things go slow, ever 100 iterations
linear=True
correlated=True
...
# Poisson equation
problem = de.IVP(domain, variables=['phi','u','v','w','T','wx','wy','Tx','Ty','Lw','LT'])
...
problem.add_equation("dx(Tx) + dy(Ty) - LT = 0")

# handle zonal flows differently if needed
if(linear):
	problem.add_equation("dt(w) + nu*dy(dy(Lw)) + nu*dx(dx(Lw)) - a*(phi - T)       = 0", condition="(ny != 0)")
	problem.add_equation("dt(T) + nu*dy(dy(LT)) + nu*dx(dx(LT)) - a*(phi - T) - k*v = 0", condition="(ny != 0)")
	problem.add_equation("dt(w) + muZF*w = 0", condition="(nx != 0) and (ny == 0)")
	problem.add_equation("dt(T) + muZF*T = 0", condition="(nx != 0) and (ny == 0)")
else:
	problem.add_equation("dt(w) + nu*dy(dy(Lw)) + nu*dx(dx(Lw)) - a*(phi - T)       = -u*wy - v*wx", condition="(ny != 0)")
	problem.add_equation("dt(T) + nu*dy(dy(LT)) + nu*dx(dx(LT)) - a*(phi - T) - k*v = -u*Ty - v*Tx", condition="(ny != 0)")
	problem.add_equation("dt(w) + muZF*w = -u*wy - v*wx", condition="(nx != 0) and (ny == 0)")
	problem.add_equation("dt(T) + muZF*T = -u*Ty - v*Tx", condition="(nx != 0) and (ny == 0)")
...
T['g'] = 0.01*np.random.uniform(-1,1,sh)
...
snapshots.add_task("T",layout='c',name='T2')
